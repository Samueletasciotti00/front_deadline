...
export default {
  ...
  methods: {
    async submitForm() {
      ...
      if (!store.loggedIn || store.userRole !== "ADMIN") {
        this.error = "Non autorizzato";
        return;
      }
      ...
      try {
        ...
        const payload = {
          ...
          remind_at: this.form.remind_at
            ? new Date(this.form.remind_at)
            : null,
          repeat: this.form.repeat,
          status: "sospeso",
        };

        await axios.post("http://127.0.0.1:3333/deadlines", payload, {
          headers: { Authorization: `Bearer ${token}` },
        });

        this.message = "Deadline creata con successo!";
        // reset form
        this.form = {
          title: "",
          description: "",
